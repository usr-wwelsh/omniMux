...
>
  ...
  <!-- Header -->
  <div class="fs-header">
    <button class="fs-close" onclick={close} title="Close">
  ...
  <div class="fs-body">
    ...
    <div class="fs-player-panel" class:mobile-hidden={tab === 'queue'}>
      ...
      <div class="fs-controls">
        ...
        <button class="fs-btn" onclick={playPrev} title="Previous">
          <svg viewBox="0 0 24 24" width="36" height="36" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
        </button>
        <button class="fs-play-btn" onclick={togglePlay} title={$isPlaying ? 'Pause' : 'Play'}>
          {#if $isPlaying}
        ...
        <button class="fs-btn" onclick={playNext} title="Next">
          <svg viewBox="0 0 24 24" width="36" height="36" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
        </button>
        <button class="fs-mode-btn" class:active={$loop !== 'none'} onclick={cycleLoop} title={$loop === 'one' ? 'Repeat one' : $loop === 'all' ? 'Repeat all' : 'No repeat'}>
          {#if $loop === 'one'}
